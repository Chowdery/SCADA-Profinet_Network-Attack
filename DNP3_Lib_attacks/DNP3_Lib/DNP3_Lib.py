...
class DNP3(Packet):
    ...
    def pre_dissect(self, s):
        length = s[2:3] # length octet
        length_with_crc = dnp3_calc_len_crc(struct.unpack('<B', length)[0])
        s = s[:dnp3_data_link_len] + remove_dnp3_crc(s[dnp3_data_link_len:length_with_crc]) + s[length_with_crc:]
        return s

    def post_build(self, pkt, pay):
        # This is incase there are multiple DNP3 fragments in one TCP
        if self.LENGTH is not None:
            length = struct.unpack('<B', pkt[2:3])[0]  # length octet to int
            # Remove SOURCE, DEST and CONTROL from length
            pay_len = (length - (2 + 2 + 1))
        else:
            # This is only if there is a if we are building one DNP3 packet
            pay_len = len(pay)
        chunks = pay_len / chunk_len  # chunk size
        last_chunk = pay_len % chunk_len
        if last_chunk > 0:
        ...
        pay = add_dnp3_crc(pay[:pay_len]) + pay[pay_len:]
...
if __name__ == '__main__':
    ...
    dnp3_pcap = rdpcap("/root/DNP3_MITM_Lib 2/DNP3_MITM_Lib/DNP3_Lib/pcaps/unsol_3.pcap")
    pkt = dnp3_pcap[0]
    #pkt.show()
    sendp(pkt)
    # sleep(1)
    pkt = CountBinaryInputPointInsert(pkt)
    #pkt.show()
    sendp(pkt)
